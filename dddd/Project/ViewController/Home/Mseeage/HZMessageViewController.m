...
- (void)viewDidLoad {
    ...
    self.tableView.mj_header = [MJRefreshGifHeader headerWithRefreshingBlock:^{
        ...
        [self.msgVM getMsgWithRequestMode:VMRequestModeRefresh completionHandler:^(NSError *error) {
            if (error) {
                NSLog(@"%@", error);
            } else {
    ...
    self.tableView.mj_footer = [MJRefreshBackNormalFooter footerWithRefreshingBlock:^{
        [self.msgVM getMsgWithRequestMode:VMRequestModeMore completionHandler:^(NSError *error) {
            if (error) {
                NSLog(@"%@", error);
            } else {
                [self.tableView reloadData];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([[self.msgVM getMsgTypeForRow:indexPath.row] integerValue] == 1) {
        cell.iconImg.image = [UIImage imageNamed:@"img_inbite-1"];
    }
