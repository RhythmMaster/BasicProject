//
//  HZWaitcancelorderViewController.m
//  dddd
//
...
@interface HZWaitcancelorderViewController ()<UITableViewDelegate, UITableViewDataSource, UITextViewDelegate>
...
@implementation HZWaitcancelorderViewController
...
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(keyboardWillHide1:) name:UIKeyboardWillHideNotification object:nil];
    ...
    [HZOrderOperateNetManager getWaitCancelOrderWithId:_orderID completionHandler:^(id model, NSError *error) {
        NSLog(@"%@", model);
        self.model = model;
        NSLog(@"%@", [[[model objectForKey:@"CONTENT"] objectForKey:@"INFO"] objectForKey:@"cancelMsg"]);
        _infoLb.text = [[[model objectForKey:@"CONTENT"] objectForKey:@"INFO"] objectForKey:@"cancelMsg"];
...
- (void)keyboardWillShow1:(NSNotification *)aNotification
{
    ...
    CGFloat registerBottomH = self.view.y - (_refuseBtnSubmit.y + _refuseBtnSubmit.height);
    
    if (height > registerBottomH) {
        CGFloat offSet = height - registerBottomH;
...
- (void)refuseClick:(UIButton *)sender {
    ...
    [HZOrderOperateNetManager getCancelOrderOperationWithId:_orderID address:address addressXY:addressXY refuseReason:_refuserReason refuseType:_refuseorID completionHandler:^(id model, NSError *error) {
        if (error) {
            NSLog(@"%@", error);
        } else {
            NSLog(@"%@", model);
            ...
            if ([[model objectForKey:@"MSG"] isEqualToString:@"成功"]) {
                UIAlertController *alertVC = [UIAlertController alertControllerWithTitle:@"提示" message:@"取消成功!是否返回订单页" preferredStyle:UIAlertControllerStyleAlert];
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        [_tableView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(self.infoLb.mas_bottom).equalTo(10);
            make.left.right.equalTo(0);
